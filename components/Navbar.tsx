...
import { Brain, ShieldUserIcon } from "lucide-react"
...
export default function Navbar() {
  ...
  const icons = {
    ...
    actions: (
      ...
      >
        {/* Lightning bolt for action/quick tasks */}
      ...
      <div className="hidden md:block w-full fixed bg-[#2F4F4F] top-0 z-999 backdrop-blur-md border-b border-white/10 text-sm">
        <nav className="h-12 flex items-center mx-8 w-full justify-between">
          ...
          <div className="flex items-center justify-center">
            ...
            <NavigationMenu>
              <NavigationMenuList className="gap-8 flex justify-center items-center">
                {visibleItems.map((item) => (
                  <NavigationMenuItem key={item.id}>
                    {isLoggedIn && item.id === 'logout' ? (
                      ...
                      >
                        {item.label}
                        {React.cloneElement(item.icon, { className: "w-3 h-3 mt-0.5" })}
                      </button>
                    ) : (
                      ...
                      <NavigationMenuLink asChild>
                        <Link
                          href={item.path}
                          className={`flex items-center justify-center gap-2 text-[16px] transition-all hover:text-white ${isActive(item.path)
      ...
      <nav className="md:hidden gap-4 fixed bottom-6 left-1/2 -translate-x-1/2 h-16 px-2 bg-action backdrop-blur-xl rounded-full border border-black/30 shadow-xl z-999 flex justify-around items-center">
        {
          visbleMobileItems.map((item) => {
            ...
            const content = (
              <>
                {active && !isArmed && (
                  <span className="absolute inset-0 bg-black/20 rounded-full scale-100 transition-transform duration-300 -z-10" />
                )}
